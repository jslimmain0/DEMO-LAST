...
## 최근 변경 (2026-07-18) — 어시스턴트 OAuth 연결(Copilot 식) + 스킬(재사용 플로우 조각)
> ⚠ **HTTP 노드 OAuth2 인증(auth 블록)은 폐기됨**(오해로 만들었다가 제거 — 커밋 e21c265). OAuth 는 **AI 어시스턴트 인증**에 쓴다(아래). 워크플로에서 OAuth 가 필요하면 내장 스킬 '토큰 발급 + Bearer 호출'(일반 HTTP 노드 조각)을 참고.
- **어시스턴트 OAuth 연결(GitHub Copilot 식 로그인)**: 관리자 API 키 붙여넣기 대신, 사용자가 OAuth 로그인해 받은 토큰으로 AI 호출. [AssistantOAuthService](backend/src/main/kotlin/com/flowlink/assistant/AssistantOAuthService.kt) — authorization_code + **PKCE(S256)** 플로우. authorize→콜백(무인증·state 검증)→코드 교환→토큰 AES-GCM 암호화 저장(테넌트)·만료 시 refresh(원자화). [AssistantOAuthController](backend/src/main/kotlin/com/flowlink/assistant/AssistantOAuthController.kt): status/config(admin)/authorize/callback(permitAll)/disconnect. provider 설정(authorize/token URL·client_id/secret·scope)은 설정에 저장(secret 암호화). [AssistantService](backend/src/main/kotlin/com/flowlink/assistant/AssistantService.kt) LLM 자격 우선순위 = **OAuth Bearer 토큰 → env/시크릿 api-key(x-api-key) → stub**(config.authMode).
  - 프론트: AssistantPanel 헤더 **AI 연결** 버튼(→authorize 리다이렉트, 로그인 후 보던 플로우로 복귀) / **🔗 연결됨**(→해제). SkillsDialog 에 provider 설정 섹션(admin, redirect URI 표시). App 이 `?ai=connected/error` 복귀 토스트.
  - **보안**(적대적 리뷰 10확정): redirect_uri 오리진을 **서버가 요청 헤더로 확정**(클라 파라미터 조작=코드 가로채기 차단) · **PKCE** · 복귀경로 상대경로 정화(오픈 리다이렉트 차단) · refresh 락 원자화·네거티브 캐시 · expires_in 문자열 허용 · config 읽기 무부작용 · 토스트 지연발화.
- **스킬 = 재사용 플로우 조각**: "우리 flow 만드는 스킬" — 스킬은 **{nodes, edges} 조각**([Skill](backend/src/main/kotlin/com/flowlink/assistant/SkillDtos.kt)). [SkillsDialog](frontend/src/components/SkillsDialog.tsx)(어시스턴트 🧠) 라이브러리에서 **＋삽입**(캔버스에 붙여넣기 — [editorStore](frontend/src/store/editorStore.ts) `insertFragment`, 새 id·토큰 재매핑·선택) 하거나 **현재 선택을 스킬로 저장**(`selectionFragment`). 내장 5종(토큰+Bearer 호출·결제창+콜백·OTP 입력·TCP 전문·HTTP 상태검증)=[BuiltinSkills](backend/src/main/kotlin/com/flowlink/assistant/BuiltinSkills.kt). AI 어시스턴트도 프롬프트로 조각을 받아 조합. 저장은 설정 JSON(마이그레이션 없음 — AppSetting.value=text, H2 dev 는 [AppSettingSchemaFix](backend/src/main/kotlin/com/flowlink/settings/AppSettingSchemaFix.kt)로 CLOB 확장).
- **팀 지침(인스트럭션)**: `PUT /api/v1/assistant/instructions`(**admin 전용** — 전 팀원 프롬프트 주입 = 스토어드 인젝션 벡터). 스킬 CRUD 는 `GET/PUT /assistant/skills`(editor).
- 검증: 백엔드 단위 무회귀 + oauth-connect e2e 15/15(설정·authorize+PKCE·콜백 코드교환·connected·authMode oauth·returnPath·오픈리다이렉트 차단·잘못된 state·disconnect) + assistant 21/21·secret 16/16 무회귀 + 브라우저(provider 설정·헤더 AI 연결 버튼·스킬 라이브러리 ＋삽입 4→5노드) + tsc/build/oxlint.
- ⚠ OAuth 토큰은 단일 인스턴스 설정(테넌트 공유 자격). provider 는 OAuth-가능한 AI 엔드포인트/게이트웨이 전제(공개 Anthropic API 는 x-api-key). 스킬 조각 노드 id 는 삽입 시 새로 부여.
