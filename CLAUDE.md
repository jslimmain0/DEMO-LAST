...
## 최근 변경 (2026-09-10) — Mock 화면 = 서버 현황(토폴로지) 하나: 목록 제거 · 포트 중심 노드 · 워크스페이스 존 · 읽기 전용
"Mock 을 엔터프라이즈(n8n) 느낌으로 — 내 워크스페이스가 아니어도 실제 서버처럼 뭐가 떠 있는지, 워크스페이스로 묶고, 포트가 잘 보이게, 내 것이 아니면 읽기 전용, **목록은 없애고 맵이 목록 기능을 다 가질 것**" 요청. 중간안(카드 타일 → 2.5D 서버 타워 아이콘)은 사용자가 반려("아이콘 관두고 포트만 잘 보이게 + 애니메이션") — 최종은 포트 중심 노드 카드 + 버스 라인.
...
- **프론트** [MockServers](frontend/src/routes/MockServers.tsx) 재작성 — **목록/카드/HTTP·TCP 탭/워크스페이스 셀렉트/보기 전환 제거**(구 `MockFleetView`·`ServerIcon` 삭제). 페이지 = 헤더(⬇ 가져오기 · + HTTP/TCP Mock — 만들기 행에 **대상 워크스페이스 select**(`fl:workspace` 기억), slug 실시간 검사) → 현황 스트립 6(서버·서빙 중·열린 포트·요청 들어오는 중·무매칭·워크스페이스 — 4개는 클릭=필터) → 툴바(검색 `/`·필터 세그먼트 전체/켜짐/꺼짐/요청 중/★·`N/M 일치`·☑ 선택 + 일괄 바(켜기/끄기/이동 select/삭제, 모두 선택=검색 일치 중 편집 가능)) → [MockTopology](frontend/src/components/MockTopology.tsx) → 포트 맵 칩. 즐겨찾기 `fl:mockfav`(전역 1키). 노드 작업은 `onAction(server, action, arg)` 한 콜백(toggle/rename(AskDialog)/duplicate/export(MockExportDialog)/delete/fav/copyUrl/move/goFlow).
  **MockTopology**(React Flow, 드래그 없음): 허브 `fhub`(왼쪽 — 호스트·`HTTP :앱포트 /mock/…` 서빙 수·TCP 열림 수) → 존 `fzone`(오른쪽 세로 스택, zIndex −1 + `pointer-events:none`, 헤더만 잡힘 — 롤 칩·서버/서빙 수·**+ Mock**(`onCreateIn` → 만들기 행 대상 워크스페이스 프리셋)) → 스파인 `fspine`(존 가운데 세로 버스 노드, 업링크 `felbow` HV 엣지 라벨 `HTTP ×N · TCP ×M`) → 서버 노드 `fserver`(2열 그리드, 탭 엣지 `felbow` TAP = 스파인 x 에서 노드 y 로 **가로 직선만**(세로는 스파인이 그려 색이 안 겹침), TCP 탭 라벨 `:포트`). 노드 = LED·이름·kind 태그·상태·**포트 19px**(`:9091` / `:앱포트`+`/mock/slug`)·요약(라우트/규칙·요청·↗N·vN)·`?` 무매칭·`!` 실패·🔒, live=`.fl-node-live` 글로우(index.css `fl-node-glow`)+`animated` 엣지, off=점선·흐림, `data-dim`(검색/필터 비매칭 → opacity .18)·`data-selected`(링+체크박스). 호버=상세 카드(`role=tooltip`, 마지막 열은 왼쪽)+도구(⏻·⋯ 메뉴 — 즐겨찾기/URL 복사/켜기끄기/이름/복제/내보내기/↗ 워크플로 펼침/이동 select/삭제/선택; 도구 컨테이너가 click·mousedown `stopPropagation` 으로 RF `onNodeClick` 차단). `onNodeClick`: selectMode·Ctrl/Shift → 선택, 아니면 readable 만 편집기. 배치는 `buildGraph`(fleet 만 의존)+`decorated`(UI 상태 덧씌움 — 선택마다 재배치 없음), 맞춤은 `fit()` 이 `getNodesBounds` 로 직접 계산(RF fitView 는 비동기라 경쟁 — 다 들어가면 가운데, 줌 하한 0.7 에 걸리면 왼쪽 위), 캔버스 높이=내용 비례(440~1000), 미니맵 구체 색. ⚠ `useNodesState` 의 `onNodesChange` 를 꼭 넘길 것(없으면 measured 미반영 → 미니맵 빈 화면·bounds 0).
- 검증: [MockFleetTest](backend/src/test/kotlin/com/flowlink/mock/MockFleetTest.kt)(JWT 심기 — 비멤버 readable=false·라우트 비움·개수 노출, VIEWER readable+정의, OWNER, TCP 리스너 LISTENING→끄면 OFF) + 전체 스위트 그린 + 브라우저 **fleet.mjs 34**(API usedBy·페이지 구성(탭/카드/셀렉트 없음)·노드 포트/LED/글로우/?/↗·탭 라벨·존 점선/관리자/+ Mock·검색 dim·현황 필터·호버 카드·⏻ 끄기→리스너 닫힘→켜기·⋯ 이름/즐겨찾기/★ 필터/워크플로 펼침·선택 모드 일괄 끄기/켜기·Ctrl+클릭·존 + Mock→대상 워크스페이스·포트 칩/노드 클릭→편집기·콘솔 무에러) + product 24·split 45·ui 18·ctxpath 무회귀(목록 단언을 맵으로 치환). 가이드 [10장](docs/guide/10-Mock-서버.md) "서버 현황" 절 재작성(목록 절·`mocks-list.png` 삭제, 스크린샷 교체).
- ⚠ 노드 위치는 자동(드래그 저장 없음). mine 은 멤버십 기준이라 관리자도 소속 아닌 팀은 남의 존에("관리자" 배지, 편집은 가능). 다른 사용자 개인 워크스페이스 이름("개인 — {user}")이 포트 점유자로 보일 수 있음(사내 도구 전제). HTTP 게이트웨이 포트는 서버가 실제 바인딩한 포트(프록시 뒤에서는 브라우저 주소와 다를 수 있음 — 노드/포트 맵의 base URL 은 브라우저 오리진 기준). usedBy 는 30초 인덱스 캐시(새 워크플로 반영 지연). 이전 세션 문서(2026-09-09 "Mock 목록 대시보드화"·"HTTP/TCP 완전 분리 목록") 의 목록·탭·카드 서술은 **이 변경으로 대체**(편집기 부분은 유효).
