...
## 최근 변경 (2026-09-10) — Mock 서버 현황(fleet): 모든 워크스페이스를 실제 서버처럼 · 포트 맵 · 읽기 전용
"Mock 을 엔터프라이즈 느낌으로 — 내 워크스페이스가 아니어도 실제 서버처럼 뭐가 떠 있는지(n8n 느낌), 워크스페이스로 묶고, 포트 뭐 뚫려 있는지, 내 것이 아니면 읽기 전용" 요청. 폴더 대신 **워크스페이스 그룹핑**으로(사용자 방향).
- **백엔드** `GET /api/v1/mock-servers/fleet`([MockServerService.fleet](backend/src/main/kotlin/com/flowlink/mock/MockServerService.kt), DTO [MockDtos.MockFleet](backend/src/main/kotlin/com/flowlink/mock/MockDtos.kt)) — 테넌트의 **모든** 워크스페이스([WorkspaceService.listAll](backend/src/main/kotlin/com/flowlink/workspace/WorkspaceService.kt) + `isMember`) × 모든 Mock 을 한 응답에:
  `FleetWorkspace{id,name,kind,myRole(null=접근 불가),mine(멤버십 기준 — 관리자 OWNER 우회와 별개)}` · `FleetServer{…, readable, myRole, listening(실제 소켓), listenError, …}` · `FleetPort{port,kind HTTP|TCP,state LISTENING|FAILED|OFF,…}`(HTTP 게이트웨이=앱 포트(`local.server.port`→`server.port`)+켜진 HTTP 수, TCP 는 mock 별 — HTTP 먼저·포트순) · `httpPort`·`contextPath`.
  **접근 불가 워크스페이스의 Mock 은 이름·slug·종류·켜짐·포트·살아있음·개수만**(routeLabels/environment 비움) — 서빙 주소·TCP 포트는 전역 자원이라 점유를 누구나 알아야 하고, 정의는 여전히 roleFor 게이트. [TcpMockRegistry](backend/src/main/kotlin/com/flowlink/mock/TcpMockRegistry.kt) 에 `listeningPort(id)`/`bindFailure(id)`(기동 시 바인딩 실패를 기억 — 저장 시 실패는 400 롤백이지만 재기동 실패는 조용했음 → 빨간불).
- **프론트** [MockFleetView](frontend/src/components/MockFleetView.tsx) — [MockServers](frontend/src/routes/MockServers.tsx) 헤더의 `[📋 목록 | 🖧 서버 현황]`(`fl:mock:view`; 서버 현황에선 워크스페이스 셀렉트 대신 "모든 워크스페이스"): 현황 스트립 5장(서버·서빙 중·열린 포트(실패 시 빨강)·요청 들어오는 중·워크스페이스/읽기 전용 수) → **🖧 토폴로지 맵**([MockTopology](frontend/src/components/MockTopology.tsx), React Flow 캔버스 — 사용자 피드백 "서버별로 컴퓨터가 띄워져 있는 그림"이 핵심: 서버마다 **2.5D 랙 서버 SVG**(`Tower` — 윗면·옆면·앞면, 전원 LED=상태(켜지면 바닥 상태색 광)·랙 유닛 활동 LED `.fl-blink`=트래픽·HTTP/TCP 라벨·하단 TCP 포트 잭/HTTP `/mock/…`·`!` 바인딩 실패·`?` 무매칭·🔒) + **호버 상세 카드**(주소·라우트 칩·마지막 요청·버전, 마지막 열은 왼쪽에). 배치: 왼쪽 열 `fnet`(네트워크·호스트, y0) 위에 `fgw`(HTTP 게이트웨이 :앱포트, y128) 아래, 오른쪽(x430)에 워크스페이스 **존 노드**(`fzone`, zIndex −1 + `pointer-events:none` 헤더만 잡힘)를 세로로 쌓고 존마다 5열 그리드. **선은 존 단위**(서버마다 그으면 아이콘을 가로질러 지저분 — 사용자 피드백 반영): `fgw`→존 왼쪽 36%(`HTTP ×켜짐/전체`), `fnet`→존 왼쪽 64%(`TCP :포트 :포트 +N`, 정렬), smoothstep(live=animated, 전부 꺼짐=점선, 실패=빨강) — 왼쪽 거터만 타므로 교차 없음. 맞춤은 RF fitView(비동기 경쟁) 대신 `fit()` 이 `getNodesBounds` 로 직접 계산(다 들어가면 가운데, 줌 하한 0.7 에 걸려 넘치면 왼쪽 위부터). 드래그 없음·팬/줌·미니맵(구체 색)·범례, 서버 집합이 바뀔 때만 재맞춤. `onNodeClick` 으로 readable 서버만 편집기) → **포트 맵**(실제 리스너 기준 칩: ● 리스닝/○ 꺼짐 점선/✕ 바인딩 실패, TCP 칩 클릭=편집기) → **▸ 워크스페이스별 타일 보기**(토글 `fl:mock:fleetTiles`, 기본 접힘 — 사용자가 "카드가 아니라 컴퓨터"를 원해 맵을 히어로로) 워크스페이스 섹션(내 소속 먼저 펼침, 나머지는 "🔒 다른 워크스페이스" 접힘 — 롤 칩 소유/편집/읽기 전용/접근 없음/관리자, 서버·서빙 수, 포트 미니, **목록으로 →**=셀렉트 전환+목록 보기) → **서버 타일**([ServerIcon](frontend/src/components/icons.tsx) 랙 + LED(초록/빨강/회색, 트래픽 시 `.fl-led-live` 펄스) · HTTP/TCP · 주소(base URL 복사 / `host:port`) · 라우트 칩/필드·규칙 수 · 요청·vN · n8n 식 **켜기/끄기 스위치**(`role=switch`, 편집 권한만) · 🔒 읽기 전용). 접근 불가 타일은 열리지 않음(not-allowed). 5초 폴링.
- 검증: [MockFleetTest](backend/src/test/kotlin/com/flowlink/mock/MockFleetTest.kt)(JWT 심기 — 비멤버 readable=false·라우트 비움·개수 노출, VIEWER readable+정의, OWNER, TCP 리스너 LISTENING→끄면 OFF) + 전체 스위트 그린 + 브라우저 **fleet.mjs 38**(API 필드·**토폴로지: 서버 노드/존/네트워크/게이트웨이 수·:18081·HTTP 머신 서빙 중+활동 LED 깜빡임·TCP 머신 포트 잭+host:port·꺼진 머신 점선 엣지·엣지=머신+1·TCP 엣지 포트 라벨·팀 존 점선+관리자·범례/미니맵/컨트롤·머신 클릭→편집기**·포트 맵 HTTP/LISTENING/OFF 점선·타일 토글·내/다른 워크스페이스 섹션(dev 를 팀에서 빼 "관리자" 배지)·타일 LED/주소/라우트·스위치 끄기→리스너 닫힘→다시 켜기·칩/타일 클릭→편집기·보기 지속·목록으로 전환·콘솔 무에러) + product 25·split 47·ui 18 무회귀. 가이드 [10장](docs/guide/10-Mock-서버.md) "서버 현황" 절.
- ⚠ mine 은 멤버십 기준이라 관리자도 소속이 아닌 팀은 "다른 워크스페이스"에(편집은 가능 — 관리자 배지). 다른 사용자의 개인 워크스페이스 이름("개인 — {user}")이 포트 점유자로 보일 수 있다(사내 도구 전제). HTTP 게이트웨이 포트는 서버가 실제 바인딩한 포트(프록시 뒤에서는 브라우저 주소와 다를 수 있음 — 타일의 base URL 은 브라우저 오리진 기준).
