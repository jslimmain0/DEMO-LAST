...
- **프론트** [MockFleetView](frontend/src/components/MockFleetView.tsx) — [MockServers](frontend/src/routes/MockServers.tsx) 헤더의 `[📋 목록 | 🖧 서버 현황]`(`fl:mock:view`; 서버 현황에선 워크스페이스 셀렉트 대신 "모든 워크스페이스"): 현황 스트립 5장(서버·서빙 중·열린 포트(실패 시 빨강)·요청 들어오는 중·워크스페이스/읽기 전용 수) → **🖧 토폴로지 맵**([MockTopology](frontend/src/components/MockTopology.tsx), React Flow 캔버스 — 사용자 피드백 "서버별로 컴퓨터가 띄워져 있는 그림"이 핵심: 서버마다 **서버 타워 SVG**(전원 LED=상태·활동 LED `.fl-blink`=트래픽·HTTP/TCP 라벨·TCP 포트 잭·`!` 바인딩 실패·`?` 무매칭·🔒), `fnet`(네트워크·호스트) → `fgw`(HTTP 게이트웨이 :앱포트) → HTTP 서버 / `fnet` → TCP 서버(`:포트` 라벨) smoothstep 엣지(live=animated, off=점선), 워크스페이스 **존 노드**(`fzone`, zIndex −1 + `pointer-events:none` 헤더만 잡힘 — 내 소속 먼저·남의 것 점선, 헤더에 롤 칩·목록으로 →). 위치는 `buildGraph` 가 데이터에서 계산(존 좌→우 wrap, 5열 그리드), 드래그 없음·팬/줌·미니맵·범례, 서버 집합이 바뀔 때만 fitView. `onNodeClick` 으로 readable 서버만 편집기) → **포트 맵**(실제 리스너 기준 칩: ● 리스닝/○ 꺼짐 점선/✕ 바인딩 실패, TCP 칩 클릭=편집기) → **▸ 워크스페이스별 타일 보기**(토글 `fl:mock:fleetTiles`, 기본 접힘 — 사용자가 "카드가 아니라 컴퓨터"를 원해 맵을 히어로로) 워크스페이스 섹션(내 소속 먼저 펼침, 나머지는 "🔒 다른 워크스페이스" 접힘 — 롤 칩 소유/편집/읽기 전용/접근 없음/관리자, 서버·서빙 수, 포트 미니, **목록으로 →**=셀렉트 전환+목록 보기) → **서버 타일**([ServerIcon](frontend/src/components/icons.tsx) 랙 + LED(초록/빨강/회색, 트래픽 시 `.fl-led-live` 펄스) · HTTP/TCP · 주소(base URL 복사 / `host:port`) · 라우트 칩/필드·규칙 수 · 요청·vN · n8n 식 **켜기/끄기 스위치**(`role=switch`, 편집 권한만) · 🔒 읽기 전용). 접근 불가 타일은 열리지 않음(not-allowed). 5초 폴링.
