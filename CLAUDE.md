...
- **GitHub OAuth 팝업 로그인**: 관리자 API 키 붙여넣기 대신 사용자가 GitHub 로그인해 받은 토큰으로 AI 호출. [AssistantOAuthService](backend/src/main/kotlin/com/flowlink/assistant/AssistantOAuthService.kt) — authorize/token URL **GitHub 고정**, 설정은 client_id/secret/scope 만(secret AES-GCM 암호화). 헤더 **GitHub 연결** 버튼이 **팝업**을 열어 로그인 → 콜백(무인증·state 검증)이 코드 교환·토큰 저장 후 팝업 닫고 `postMessage`(same-origin)로 opener 에 결과 통지 → 상태 갱신. [AssistantService](backend/src/main/kotlin/com/flowlink/assistant/AssistantService.kt) LLM 자격 우선순위 = **OAuth Bearer 토큰 → env/시크릿 api-key(x-api-key) → stub**(config.authMode). 만료 시 refresh(락 원자화·네거티브 캐시). redirect_uri 오리진은 **서버가 요청 헤더로 확정**(클라 조작=코드 가로채기 차단). GitHub confidential client(client_secret)라 PKCE 불요.
- 검증: 백엔드 단위 무회귀 + github-prompts e2e 15/15(GitHub 설정·authorize=github.com·콜백 HTML 팝업 스크립트·잘못된 state·disconnect·프롬프트 CRUD·이름검증·지침 영속) + assistant 21/21·secret 16/16 무회귀 + 브라우저(프롬프트 라이브러리·GitHub 연결 버튼·프롬프트 ▶적용→전송·응답) + tsc/build/oxlint.
- ⚠ GitHub 토큰 교환→연결은 실제 GitHub OAuth App 자격 필요(e2e 는 도달 가능한 부분만). OAuth 토큰은 단일 인스턴스·테넌트 공유 자격. GitHub 토큰이 실제로 AI 호출에 쓰이려면 base-url 이 그 토큰을 받는 엔드포인트여야 함(공개 Anthropic API 는 x-api-key).
