...
- 검증: API(3단 중첩 생성·parentId 반영·삭제 시 상위 승격) + 브라우저 e2e 18(타일 진입·브레드크럼 이동·폴더 안 새 폴더·
  한 줄 높이 유지·가로 스크롤·⤢ 표시/미표시·다이얼로그 편집 동기화) PASS, 기존 e2e 40(14+12+10+4) 무회귀.
- ⚠️ 폴더 재배치(드래그로 폴더 이동)·flowCount 는 직속 워크플로 수만(하위 합산 아님) — 후속. 사이드바 트리는 항상 펼침(접기 없음).
