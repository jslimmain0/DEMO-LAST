...
## 최근 변경 (2026-07-18) — HTTP OAuth2 인증 + 스킬(재사용 플로우 조각)
- **HTTP 노드 OAuth2 인증**: `auth` 블록 `{type:oauth2_cc, tokenUrl, clientId, clientSecret, scope, clientAuth}`([HttpAuth](backend/src/main/kotlin/com/flowlink/core/graph/HttpAuth.kt)).
  [OAuthTokenProvider](backend/src/main/kotlin/com/flowlink/execution/engine/OAuthTokenProvider.kt) 가 실행 직전 client_credentials 토큰을 취득·캐시(tenant+시크릿 SHA-256 키·만료 30초 스큐·응답 1MB 상한·evict)해 `Authorization: Bearer` 주입(**server 모드 전용** — client 모드는 노드 실패로 무인증 전송 방지). clientSecret 은 `{{ 이름@secret }}` 권장(토큰은 requestText 미포함=로그 유출 없음). tokenUrl SSRF 검증. PropertyPanel '인증' 섹션(+평문 경고), NodeCard 🔒.
- **스킬 = 재사용 플로우 조각**: "우리 flow 만드는 스킬" — 스킬은 **{nodes, edges} 조각**([Skill](backend/src/main/kotlin/com/flowlink/assistant/SkillDtos.kt)). [SkillsDialog](frontend/src/components/SkillsDialog.tsx)(어시스턴트 🧠) 라이브러리에서 **＋삽입**(캔버스에 붙여넣기 — [editorStore](frontend/src/store/editorStore.ts) `insertFragment`, 새 id·토큰 재매핑·선택) 하거나 **현재 선택을 스킬로 저장**(`selectionFragment`). 내장 5종(OAuth 인증 호출·결제창+콜백·OTP 입력·TCP 전문·HTTP 상태검증)=[BuiltinSkills](backend/src/main/kotlin/com/flowlink/assistant/BuiltinSkills.kt). AI 어시스턴트도 프롬프트로 조각을 받아 조합. 저장은 설정 JSON(마이그레이션 없음 — AppSetting.value=text, H2 dev 는 [AppSettingSchemaFix](backend/src/main/kotlin/com/flowlink/settings/AppSettingSchemaFix.kt)로 CLOB 확장).
- **팀 지침(인스트럭션)**: `PUT /api/v1/assistant/instructions`(**admin 전용** — 전 팀원 프롬프트 주입 = 스토어드 인젝션 벡터). 스킬 CRUD 는 `GET/PUT /assistant/skills`(editor).
- **적대적 리뷰(OAuth+스킬 6확정) 반영**: OAuth 토큰 응답 상한(OOM)·캐시 키 SHA-256+tenant(hashCode 충돌 방지)·evict·client 모드 거절 · redactSecrets 가 OAuth clientSecret 리터럴도 마스킹 · 프롬프트 길이 상한 · 이름없는 스킬 400 · 지침 admin 게이트 · SkillsDialog 최초 1회 시드(refetch 편집 유실 방지).
- 검증: 단위(AssistantJsonTest 6·무회귀) + oauth/skills e2e 16/16(토큰 취득/주입/캐시 1회·client FAILED·조각 CRUD·이름검증·지침 영속) + assistant e2e 21/21 무회귀 + 브라우저(인증 섹션·OAuth 필드·스킬 라이브러리·＋삽입 4→5노드) + tsc/build/oxlint.
- ⚠ OAuth client_secret 평문 리터럴은 그래프에 저장·LLM 전송 시 마스킹만(볼트 토큰 권장). 토큰 캐시는 단일 인스턴스 인메모리. 스킬 조각의 노드 id 는 삽입 시 새로 부여(토큰 재매핑).
