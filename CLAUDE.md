...
- **TCP 수신 실패 가시성(버그 수정)**: 응답 단계 전에 죽은 요청(요청 코덱 실패·길이 프리픽스 불일치·불완전 수신)이 **요청 기록에 아무것도 안 남고 조용히 끊기던** 결함 — 프레임 읽기를 [TcpMockRegistry.readFrame](backend/src/main/kotlin/com/flowlink/mock/TcpMockRegistry.kt)(`Ok|Closed|Bad(reason, received)` — 실패해도 받은 바이트 보존)으로 분리하고, `JournalEntry.error`(+`MockRequestLog.error`)로 사유·받은 전문·hex 를 기록(프레이밍 400·코덱/그 외 500) + WARN 로그. 편집기 전문/요청 기록 행이 빨갛게 `⚠ 사유`(헤더 `⚠ 실패 N`). ⚠ 정상 종료(아무 바이트도 안 온 연결·유휴 타임아웃)는 기록하지 않는다(잡음 방지). 단위 `TcpFrameReadTest` 9 + 라이브 e2e 23.
- **전문 길이 토큰 `{{len}}`(신규)**: 전문 **안**의 길이 필드를 손으로 안 적게 — 순수 [TcpLen](backend/src/main/kotlin/com/flowlink/common/tcp/TcpLen.kt) 하나를 노드/Mock 이 공유. `{{len}}`=본문 바이트(**고정길이라 선언 길이의 합**) · `{{len:N}}`=N자리 0 패딩(넘치면 하위 N자리 — 폭 불변) · `{{len:frame}}`·`{{len:frame:N}}`=본문+프리픽스 폭. 적용: [TcpNodeExecutor](backend/src/main/kotlin/com/flowlink/execution/engine/TcpNodeExecutor.kt) 요청 필드(패딩 전, TokenResolver 보다 **먼저**) · [TcpMockEngine](backend/src/main/kotlin/com/flowlink/mock/TcpMockEngine.kt) 응답 필드(선언 합) / 템플릿 모드(**2패스** — 자리표시자 0 으로 폭 확정 → 바이트 측정 → 같은 폭의 숫자로 재치환, bare 폭은 고정점; [MockTemplate](backend/src/main/kotlin/com/flowlink/mock/MockTemplate.kt) 이 TCP 문맥에서 길이 토큰을 원문 보존) · 미리보기 2종 동일 경로. **bare `{{len}}` 은 상위 노드에 `len` 출력이 있으면 그 값이 우선**(시각 토큰 now/today/time 과 같은 규약, `TokenResolver.upstreamValue` 재사용), 명시형은 항상 길이. 원문 유지는 `:` 이 들어간 잘못된 형태(`{{len:x}}`·`{{len:1234}}`)뿐 — `{{length}}`·`{{LEN}}` 은 길이 토큰이 아니라 **TokenResolver 의 일반 bare 바인딩**이라 상위 값이 없으면 빈 값이다(프론트 [tokenGrammar](frontend/src/lib/tokenGrammar.ts) 미러도 같은 합집합). 단위 TcpLenTest 13·TcpMockLenTest 7·TcpNodeLenTest 7. ⚠ Mock 응답에 `target=body` 코덱이 걸리면 길이는 **코덱 적용 전** 본문 기준. 가이드 [03장](docs/guide/03-노드-레퍼런스.md)·[10장](docs/guide/10-Mock-서버.md).
