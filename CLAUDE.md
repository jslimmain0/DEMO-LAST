...
- **GitHub OAuth 팝업 로그인 → GitHub Models 게이트웨이로 AI 호출**: 관리자 API 키 붙여넣기 대신 사용자가 GitHub 로그인해 받은 토큰으로 AI 호출. [AssistantOAuthService](backend/src/main/kotlin/com/flowlink/assistant/AssistantOAuthService.kt) — authorize/token URL **GitHub 고정**, 설정은 client_id/secret/scope + **게이트웨이(baseUrl·model)** (secret AES-GCM). 헤더 **GitHub 연결** 버튼이 **팝업**을 열어 로그인 → 콜백(무인증·state 검증)이 코드 교환·토큰 저장 후 팝업 닫고 `postMessage`(same-origin)로 opener 에 통지 → 상태 갱신.
  - **게이트웨이**: GitHub 토큰(Bearer)으로 **GitHub Models**(OpenAI 호환 `/chat/completions`, 기본 `https://models.github.ai/inference` · `openai/gpt-4o`, admin 변경 가능)를 호출. [AssistantService](backend/src/main/kotlin/com/flowlink/assistant/AssistantService.kt) 가 자격에 따라 포맷 분기: **OAuth→OpenAI(Bearer·system 첫 메시지·choices[0].message.content)** / **api-key→Anthropic(x-api-key·/v1/messages·content[].text)**. 자격 우선순위 OAuth→key→stub. 만료 시 refresh(락 원자화·네거티브 캐시). redirect_uri 오리진은 서버가 요청 헤더로 확정(코드 가로채기 차단), GitHub confidential client 라 PKCE 불요.
- 검증: 백엔드 단위(AssistantFormat 5·포맷 변환) + github-prompts e2e 17/17(GitHub 설정·게이트웨이 기본/오버라이드·authorize=github.com·콜백 HTML 팝업·잘못된 state·disconnect·프롬프트 CRUD·지침 영속) + assistant 21/21·secret 16/16 무회귀 + 브라우저(프롬프트 라이브러리·GitHub 연결 버튼·프롬프트 ▶적용→전송·게이트웨이 필드·샘플 칩 제거) + tsc/build/oxlint.
- ⚠ GitHub 토큰 교환·GitHub Models 라이브 호출은 실제 GitHub OAuth App 자격 + 적절 scope 필요(e2e 는 설정/포맷/도달 가능한 부분까지). OAuth 토큰은 단일 인스턴스·테넌트 공유 자격.
