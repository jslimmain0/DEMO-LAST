...
/**
 * 앱 자체 JWT([AppJwt], Keycloak 호환 클레임 구조)의 롤 클레임을 Spring Security 권한으로 매핑.
 *
 ...
 *   scope 클레임만 보므로 realm_access/resource_access 롤은 커스텀 매핑이 필요하다)
 * - principal name 은 `preferred_username`(없으면 sub) — 실행 이력 triggeredBy 에 사용.
 *
 * 클레임 형태가 예상과 다르면(리스트가 아님 등) 조용히 무시한다 —
