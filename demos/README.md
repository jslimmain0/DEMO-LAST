...
> **두 가지 mock 방식**: 이 폴더의 데모 01~06 은 리포 루트의 손수 작성 스크립트 **`mock-server.js`**(:9090)를 대상으로 한다.
> 그와 별개로 FlowLink 에는 **내장 Mock 서버 기능**(상단 "Mock 서버" 탭 — 저장 즉시 `/mock/{slug}/**` 서빙)이 있다.
> 내장 기능으로 "결제창 뜨고 콜백" 흐름을 만드는 데모는 [`pay-mock/`](pay-mock/README.md) 폴더에 있다.
> 미완성 API 를 UI 에서 바로 mock 으로 세우려면(임의 응답·조건 분기·콜백 발사) 내장 기능을 쓰면 된다.
