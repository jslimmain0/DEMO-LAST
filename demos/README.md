# FlowLink 데모 워크플로 — mock-server.js 대상 핵심 흐름

`mock-server.js`(가짜 대상 시스템)를 대상으로 HTTP/폼/콜백/TCP/변환 등 핵심 노드 흐름을 돌려보는 데모 6종.

> **두 가지 데모 갈래**:
> - **이 폴더(01~06)** — 리포 루트의 손수 작성 스크립트 **`mock-server.js`**(:9090/:9091)를 대상으로 하는 기본 투어(TCP·상태 있는 흐름 포함).
> - **[`pay-mock/`](pay-mock/README.md)** — FlowLink **내장 Mock 서버 기능**(상단 "Mock 서버" 탭, `/mock/{slug}/**`)과 **검증(assert) 노드**를 쓰는 다음 단계 데모("결제창 뜨고 콜백" 흐름). 미완성 API 를 UI 에서 바로 mock 으로 세우는 법은 여기서 다룬다.
