...
- **탭**: `🌐 HTTP Mock | 🔌 TCP Mock`(개수 배지, 요청 들어오는 중이면 초록 점). 만들기도 **+ HTTP Mock** / **+ TCP Mock** 으로 따로.
- **현황 스트립**(현재 탭 기준): **전체 · 서빙 중(리스너 열림) · 요청 들어오는 중(최근 60초) · 무매칭 요청 있음** — 카드를 클릭하면 그 조건으로 필터(다시 클릭하면 해제). 무매칭이 있으면 빨갛게 — 규칙이 없어 404/빈 응답이 나간 Mock 입니다.
- **⚡ 최근 트래픽**: 요청을 받은 Mock 을 마지막 요청 순으로 5개(기록 수·무매칭 수). 클릭하면 열립니다.
- **카드(2열)**: 이름·`◈ 코덱`·`🔑 시크릿 환경`·`레거시` 배지, **살아있음 점**, 켜짐/꺼짐 토글, ⋯ 메뉴.
  - HTTP: base URL(클릭 복사) + **라우트 미니 스트립**(메서드 색 점 + 경로, 앞 8개 + `+N`).
  - TCP: `:포트`(리스너 꺼짐 표시) + **요청 필드 수 · 규칙 수 · 전문 코덱** 필.
  - 푸터: 마지막 요청·기록 수·**무매칭 N**·**↗ 워크플로 N**(이 base URL 을 쓰는 워크플로 — 클릭하면 목록/이동)·`vN`·수정 시각.
- **검색**(`/` 또는 Ctrl+F): 이름·slug·경로(HTTP)/포트(TCP). **정렬**: 최근 수정 / 이름 / 요청 많은 순. **필터**: 전체·켜짐·꺼짐.
- **⋯ 메뉴**: ★ 즐겨찾기(상단 고정) · ✎ 이름 바꾸기 · ⧉ 복제(slug `-2`, TCP 는 리스너 꺼서) · ⬆ 내보내기 · 워크스페이스 이동 · 🗑 삭제(사용 중인 워크플로 수를 경고).
- **☑ 선택**: 현재 탭의 여러 Mock 을 골라 한 번에 켜기/끄기/삭제. **⬇ 가져오기**(내보낸 JSON 붙여넣기)는 유형을 JSON 에서 읽습니다.

끄면 서빙이 중단되어, 그 mock 을 호출하면 404("mock 서버가 없거나 비활성화됨") JSON 에러가 옵니다(TCP 는 리스너가 닫힘).

## 서버 현황 — 모든 워크스페이스를 실제 서버처럼

목록 상단의 `[📋 목록 | 🖧 서버 현황]` 에서 **서버 현황**을 고르면, 워크스페이스 선택과 무관하게 **모든 워크스페이스의 Mock** 이 캔버스 위에 실제 서버처럼 나옵니다(5초마다 갱신).

![서버 현황](images/mocks-fleet.png)

- **현황 스트립**: 서버(HTTP/TCP 수) · 서빙 중 · **열린 포트**(바인딩 실패가 있으면 빨갛게) · 요청 들어오는 중(최근 60초) · 워크스페이스(읽기 전용 수).
- **🖧 토폴로지(캔버스)**: 왼쪽에 `네트워크(호스트)` 와 `HTTP 게이트웨이 :앱포트` 장비, 오른쪽에 워크스페이스 **존(영역)** 이 세로로 쌓이고 존마다 **서버 컴퓨터(2.5D 랙 서버)** 가 서 있습니다. 선은 존 단위 — 게이트웨이 → 존(`HTTP ×켜진 수/전체`), 네트워크 → 존(`TCP :9091 :19391 +N` 열린 포트) — 라벨이 왼쪽 거터에 붙고 서버를 가로지르지 않습니다. 내 소속 존이 먼저, 남의 것은 점선 존에 🔒.
  서버 아이콘: 상단 `HTTP`/`TCP` 라벨 + **전원 LED**(초록=서빙 중/리스닝 · 빨강=바인딩 실패 · 회색=꺼짐, 켜져 있으면 바닥에 상태색 광), 랙 유닛의 **활동 LED 는 요청이 들어오면 깜빡이고 선이 흐릅니다**, 하단은 TCP **포트 잭**(`:9091`) / HTTP `/mock/…`. 꺼진 서버는 흐리게. `!`=바인딩 실패, `?`=무매칭 요청 있음. 마우스를 올리면 **상세 카드**(주소·라우트 목록·마지막 요청·버전), **클릭하면 편집기**(접근 권한 없는 서버는 열리지 않음). 휠=이동, Ctrl+휠=줌, 우하단 미니맵·좌하단 맞춤(⛶). 존 헤더의 **목록으로 →** 로 그 워크스페이스 목록 보기.
- **🔌 포트 맵**: *지금 실제로 열려 있는 소켓* 기준입니다. `:앱포트 HTTP 게이트웨이 · N개 서빙`(모든 HTTP Mock 이 `/mock/{slug}` 로 나가는 포트)이 먼저, 그 뒤 TCP Mock 마다 `:포트` 칩 — **● 리스닝** / **○ 꺼짐**(점선) / **✕ 바인딩 실패**(켜져 있는데 포트를 못 연 경우, 예: 재기동 때 다른 프로세스가 선점). 칩을 클릭하면 그 Mock 편집기가 열립니다.
- **▸ 워크스페이스별 타일 보기**(맵 아래 토글): 같은 정보를 카드로 — 내 소속(🌐 공용 · 🔒 내 개인 · 👥 멤버인 팀)이 먼저, 나머지는 **🔒 다른 워크스페이스**로 접혀 있습니다. 헤더에 내 롤(소유 / 편집 / 읽기 전용 / 접근 없음)·서버 수·서빙 수·포트 목록, **목록으로 →**.
  타일: 랙 아이콘 + 상태 LED · `HTTP`/`TCP` · 주소(HTTP 는 base URL 클릭 복사, TCP 는 `호스트:포트`) · 라우트 칩 또는 요청 필드·규칙 수 · 마지막 요청 · `vN`. 편집 권한이 있으면 **켜기/끄기 스위치**, 타일을 클릭하면 편집기.
- **읽기 전용**: 내 워크스페이스가 아니면(VIEWER 포함) 스위치가 없고 🔒 가 붙습니다 — 열어서 볼 수는 있습니다. **접근 권한이 없는 워크스페이스**의 서버는 이름·종류·켜짐·포트·살아있음만 보이고(서빙 주소와 TCP 포트는 전역 자원이라 누가 어떤 포트를 쓰는지는 알 수 있어야 하므로) 라우트·환경 같은 정의는 비공개이며 열리지도 않습니다. 관리자는 모든 워크스페이스를 편집할 수 있지만 소속이 아닌 팀은 "관리자" 배지와 함께 다른 워크스페이스 쪽에 나옵니다.
- 새 Mock(**+ HTTP Mock / + TCP Mock**)은 서버 현황에서도 만들 수 있고, 목록 보기에서 마지막으로 골라 둔 워크스페이스에 생성됩니다.
