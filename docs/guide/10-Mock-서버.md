...
- **🖧 토폴로지(캔버스)**: 서버마다 **컴퓨터(서버 타워)** 가 서 있고, `네트워크(호스트)` → `HTTP 게이트웨이 :앱포트` → HTTP 서버, `네트워크` → TCP 서버(선에 `:포트` 라벨)로 선이 이어집니다. 워크스페이스는 **존(영역)** 으로 묶여 내 소속이 먼저, 남의 것은 점선 존에 🔒. 타워의 전원 LED(초록=서빙 중/리스닝 · 빨강=바인딩 실패 · 회색=꺼짐)와 상단 `HTTP`/`TCP` 라벨, TCP 는 하단 **포트 잭**(`:9091`), 요청이 들어오면 **활동 LED 가 깜빡이고 선이 흐릅니다**. 꺼진 서버는 흐리게 + 점선. `!`=바인딩 실패, `?`=무매칭 요청 있음. 휠=이동, Ctrl+휠=줌, 우하단 미니맵·좌하단 맞춤(⛶). **서버를 클릭하면 편집기**(접근 권한 없는 서버는 열리지 않음), 존 헤더의 **목록으로 →** 로 그 워크스페이스 목록 보기.
