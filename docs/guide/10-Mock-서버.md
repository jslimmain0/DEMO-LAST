...
## 서버 인벤토리 — Mock 화면은 행 목록 하나

사이드바 → **Mock 서버**. 카드도 그래프도 없이, **모든 워크스페이스의 Mock 을 실제 서버처럼** 한 줄에 하나씩 보여 주는 화면 하나입니다(Docker Desktop 의 컨테이너 목록을 떠올리면 됩니다. 5초마다 갱신).

![서버 인벤토리](images/mocks-fleet.png)
...
- **툴바**: 검색(`/` 또는 Ctrl+F — 이름·slug·경로·포트·워크스페이스), 필터(전체/켜짐/꺼짐/요청 중/★ 즐겨찾기), **☑ 선택**(일괄 켜기·끄기·워크스페이스 이동·삭제 — "모두 선택"은 검색에 걸린 것 중 편집 가능한 것만, Ctrl+클릭으로도 선택). 검색/필터에 안 맞는 행은 **숨겨지고** 그룹 헤더에 `서버 1/15` 처럼 남아 `N/M 일치`가 표시됩니다.
- **포트 스트립**(맨 위): *지금 실제로 열려 있는 소켓* 기준 칩 — `:앱포트 HTTP 게이트웨이 ×N` + 리스닝 중인 TCP Mock 마다 `:포트 이름`(● 리스닝 / ✕ 바인딩 실패 — 켜져 있는데 포트를 못 연 경우, 예: 재기동 때 다른 프로세스가 선점). 꺼진 TCP 포트는 `▸ 꺼짐 N` 으로 접혀 있습니다. **칩 클릭 = 그 행으로 스크롤 + 하이라이트**.
- **워크스페이스 그룹**: 내 소속(공용·내 개인·멤버인 팀)이 먼저 펼쳐지고, 남의 것은 접힌 채 🔒(읽기 전용/접근 없음). 헤더에 롤 칩(소유/편집/읽기 전용/접근 없음/관리자) · `서버 N · 서빙 M · 요청 중 K` · TCP 포트 미니 칩 · **+ Mock**(그 워크스페이스에 만들기). 접힘 상태는 브라우저가 기억하며, 검색 중엔 일치가 있는 그룹만 자동으로 펼쳐집니다.
- **행** = 서버 하나: LED(◉ 요청 들어오는 중이면 펄스 · ● 서빙/리스닝 · ✕ 바인딩 실패 · ○ 꺼짐) → **포트가 제일 크게**(TCP `:9091 localhost`, HTTP `:앱포트 /mock/slug`) → 이름(★ 즐겨찾기) → `HTTP`/`TCP` · 상태(서빙 중/리스닝/꺼짐/바인딩 실패 `!`, 🔒) → 구성(HTTP 는 라우트 필 `POST /pay` 최대 3 + `+N`, TCP 는 `필드 N · 규칙 M`, ◈ 코덱 · 🔑 시크릿 환경) → 트래픽(`● N건/60초 · 마지막 요청 · 무매칭 N`) → `↗N`(이 Mock 을 쓰는 워크플로 수) → `vN` → **⏻**(켜기/끄기) **⋯**(★ 즐겨찾기 · base URL 복사 · 켜기/끄기 · 이름 바꾸기 · 복제 · 내보내기 · ↗ 워크플로 목록(클릭하면 그 워크플로로) · 워크스페이스로 이동 · 삭제 · 선택). 도구는 항상 보입니다(호버 불필요).
  **행 클릭 = 편집기**(Enter 도 됨), Ctrl/Shift+클릭 = 선택. 꺼진 행은 흐리게, 접근 권한 없는 행은 열리지 않습니다.
- **읽기 전용**: 내 워크스페이스가 아니면(VIEWER 포함) ⏻·편집 메뉴가 없고 🔒 — 열람만. **접근 권한이 없는 워크스페이스**의 서버는 이름·종류·켜짐·포트·살아있음·개수만 보이고(서빙 주소와 TCP 포트는 전역 자원이라 누가 어떤 포트를 쓰는지는 알 수 있어야 하므로) 라우트·환경 같은 정의는 "정의 비공개"로 가려지며 열리지도 않습니다. 관리자는 편집할 수 있지만 소속이 아닌 팀은 "관리자" 배지로 남의 그룹에 나옵니다.
...
- **요청 레이아웃**: 들어온 전문을 앞에서부터 **바이트 길이**대로 잘라 필드명을 붙입니다(오프셋 자동 표시). 이 이름으로 규칙 조건과 `{{ 필드@req }}` 토큰을 씁니다. 필드마다 **◈** — 그 필드가 암호화/인코딩돼 오면 **요청 전 풀기** 코덱을 겁니다.
  **[필드 | 텍스트]** 로 표 ⇄ 한 줄 문법(`전문코드 4` · `고객명 10 EUC-KR`)을 오갈 수 있고, **📋 정의서 붙여넣기** 로 엑셀 정의서(항목명/길이/타입/기본값)를 한 번에 옮깁니다 — 문법과 다이얼로그 사용법은 [03장 TCP 전문](03-노드-레퍼런스.md#tcp-전문)과 같습니다(**⧉ 텍스트 복사** 로 노드↔Mock 사이 복사).
- **규칙 상세**: `요청에 포함`(contains) AND **필드 조건**(`전문코드 eq 0200` 등 — eq/ne/contains/startswith/endswith/regex/exists)이 모두 맞는 첫 규칙. 둘 다 비면 기본 규칙.
  **응답 전문 = [필드 | 텍스트] + [템플릿(고급)]**:
  ...
  - **텍스트**: 같은 필드 정의를 한 줄 문법으로 본 것(`응답코드 4 문자 = 0000` · `잔액 12 숫자 = 1500000`) — **무손실 왕복**이라 텍스트로 고치고 [필드]로 돌아오면 그대로 반영됩니다. 여기서도 **📋 붙여넣기 / ⧉ 복사**.
  - **템플릿(고급)**: 필드 정의를 버리고 평문 한 줄 템플릿(`0000{{req.계좌번호}}…`)으로 — 길이를 직접 맞춰야 합니다. 필드가 있는 상태에서 누르면 `필드 N개 정의를 버립니다` **확인/취소** 를 한 번 더 받습니다(되돌리려면 [필드 모드로]).
- 값 템플릿: `{{req.필드명}}`(=`{{ 필드명@req }}`) 요청 필드 · `{{req}}` 전체 · `{{req:오프셋:길이}}` 바이트 슬라이스 · `{{ 이름@secret }}` 시크릿 · `{{seq}}` `{{now}}` `{{uuid}}`.
- **전문 안 길이 필드**(`{{len:4}}`): 응답 전문에도 길이 필드가 있으면 값에 토큰을 넣어 자동으로 채웁니다 — **필드 모드**는 `응답 필드 선언 길이의 합`, **템플릿(고급) 모드**는 렌더가 끝난 본문의 실제 바이트 수로 채웁니다. `{{len:frame:4}}` 는 프리픽스까지 포함한 값(규칙 상세의 `응답 본문 40B + 프리픽스 4B = 나가는 전문 44B` 줄의 두 숫자). 이름이 길이처럼 보이는 행에는 **`{{len}}` 버튼**이 붙습니다. 워크플로 TCP 노드와 **같은 토큰**입니다([03장](03-노드-레퍼런스.md#tcp-전문)).
  ⚠ 응답 **본문 전체**에 코덱(`target=body`, 예: base64 인코딩)이 걸려 있으면 길이는 **코덱 적용 전** 본문 기준입니다.
- **하단 트래픽 패널** = **[전문 기록 | 🔍 전문 미리보기]**.
  - **전문 기록**: 실제로 들어온 전문(디코딩 텍스트, 바이트 수 → 응답 바이트 수, 매칭 규칙/무매칭, 코덱 적용 후). 행의 **[🔍 이 전문으로 미리보기]** 가 그 전문을 샘플로 넘깁니다.
  - **응답 전에 실패한 요청도 남습니다** — **요청 코덱 실패**(플러그인 없음/예외) · **길이 프리픽스 불일치**(숫자가 아님·선언 길이가 상한 초과) · **불완전 수신**(선언한 본문 길이만큼 안 옴, 상대가 연결을 안 닫음). 행이 빨갛게 뜨고 **⚠ 사유**(고칠 설정 이름까지: 예 `본문 200바이트를 기다렸지만 196바이트만 도착했습니다 — 보낸 쪽의 길이 값이 자기 자신을 포함한다면 '프리픽스 포함 길이'를 켜세요`)와 **받은 바이트(전문·hex)** 를 함께 보여줍니다. 헤더 줄의 `⚠ 실패 N` 이 개수입니다. 예전에는 이런 요청이 아무 흔적 없이 끊겼습니다.
...
- 워크플로의 **TCP 전문 노드**([03장](03-노드-레퍼런스.md#tcp-전문))의 대상을 `127.0.0.1:{포트}` 로 잡으면 끝(헤더의 `host:port ⧉` 복사). 새 TCP Mock 은 빈 포트를 골라 레이아웃(전문코드4·계좌10)+필드 응답 예시로 시작하며, **새 TCP 노드의 기본값도 같은 전문**이라 그냥 연결해도 첫 실행이 성공합니다.
- **부르는 노드를 여기서 만들기** — ⋯ 도구:
  - **▶ 이 Mock 을 부르는 TCP 노드 만들기 (새 워크플로)** — 이 Mock 의 거울(연결·요청 레이아웃·응답 필드)을 가진 TCP 노드를 새 워크플로에 만들고 에디터로 이동합니다.
  - **⧉ TCP 노드 복사 (에디터에서 Ctrl+V)** — 같은 노드를 클립보드에 담아 이미 만들어 둔 워크플로 캔버스에 붙여넣습니다.
  - 둘 다 **미저장 편집이 있으면 먼저 저장**합니다(노드는 저장된 정의의 거울이라야 하므로). 만들어진 노드에는 `🔌 Mock '이름' :포트 · 리스닝` 과 `✓ 레이아웃 일치` 칩이 붙습니다.
...
| **`{{len:4}}`** `{{len:frame:4}}` | — | **TCP 전용** — 전문 안 길이 필드. 본문 바이트 / 프리픽스 포함 전체 바이트를 N자리 0 패딩으로 자동 |
