...
when_to_use: "트리거: 'FlowLink 워크플로 만들어줘', '~하는 플로우 짜줘', '결제/OTP/주문 워크플로 생성', '이 흐름을 노드로'. FlowLink(REST API 워크플로 오케스트레이션) 대상."
---
...
4. **JSON 작성 & 등록** — `{ name, nodes, edges }` 그래프를 파일(또는 stdin)로 만들어 헬퍼로 등록:
   ```bash
   node "${CLAUDE_SKILL_DIR}/scripts/register-flow.mjs" /path/to/graph.json
