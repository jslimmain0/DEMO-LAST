...
export function makeNode(type: NodeType, x: number, y: number): GraphNode {
  ...
  switch (type) {
    ...
    case 'form':
      return {
        id, name: '폼 전송', type: 'form', cat: 'wait', x, y,
        formAction: 'https://example.com/pay', formMethod: 'POST',
        fields: { params: [], headers: [], body: [{ id: newId(), key: 'field1', value: '' }] },
      }
    case 'wait':
      return {
        id, name: '콜백 대기', type: 'wait', cat: 'wait', x, y,
        waitTimeoutSec: 120, cbRespType: 'text', cbRespBody: 'OK',
        outputs: [{ key: 'resultCode', type: 'string' }],
      }
    case 'input':
      return {
        id, name: '입력 대기', type: 'input', cat: 'wait', x, y,
...
export const PALETTE: Array<{ type: NodeType; label: string; cat: string }> = [
  ...
  { type: 'form', label: '폼 전송', cat: 'wait' },
  { type: 'wait', label: '콜백 대기', cat: 'wait' },
  { type: 'input', label: '입력 대기', cat: 'wait' },
