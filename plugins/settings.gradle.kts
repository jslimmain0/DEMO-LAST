...
//
// :spi 는 앱 안(backend/src/main/kotlin/com/flowlink/transform/FlowTransform.kt)과 **같은 규격의 사본**이다.
// 플러그인은 이걸 compileOnly 로만 물고(JAR 에 미번들), 런타임엔 앱이 제공하는 같은 FQCN 을 쓴다.
// → 이 디렉터리만 복사해 가도 리포 밖에서 그대로 빌드된다.
